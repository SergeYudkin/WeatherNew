package com.example.weathernew.utils

const val BUNDLE_KEY = "key"
const val API_KEY = "X-Yandex-API-Key"
const val BUNDLE_KEY_WEATHER = "key_weather_DTO"
const val BROADCAST_KEY = " broadcast_key"
const val BROADCAST_ACTION = " broadcast_action_key"
const val DETAILS_SERVICE_KEY_EXTRAS = "key"
const val BUNDLE_KEY_LAT = "key_lat"
const val BUNDLE_KEY_LON = "key_lon"
const val DETAILS_INTENT_FILTER = "DETAILS INTENT FILTER"
const val YANDEX_API_URL = "https://api.weather.yandex.ru/"
const val YANDEX_API_URL_END_POINT = "v2/informers"
const val DB_NAME = "History.db"
const val KEY_SP = "sp"
const val KEY_CURRENT_CITIES = "currentCities"
const val REQUEST_CODE_CONT = 777
const val REQUEST_CODE_CALL = 888
const val REQUEST_CODE_FINE_LOCATION = 999
const val MIN_DISTANCE = 100f     //  100 метров это расстояние на которое отреагирует лисенер
const val REFRESH_PERIOD = 60000L  // лисенер будет обновляться каждые 60 секунд
const val ID = "id"
const val NAME = "name"
const val TEMPERATURE = "temperature"
const val FEELS_LIKE = "feelsLike"
const val ICON = "icon"
